...
public class TestGame extends GameModel {
	...
	public Object paddleLock=new Object();
	...
	/*
	 * public void start(){ timer=new Timer(); timer.scheduleAtFixedRate(new
	 * TimerTask(){
	 * 
	 * @Override public void run() { ball.move(); if(paddle1.collision(ball) ||
	 * paddle2.collision(ball)){ ball.setVelocity(ball.velx, ball.vely,
	 * -1*ball.velz); } if(ball.x<-25||ball.x>25){
	 * ball.setVelocity(-1*ball.velx, ball.vely, ball.velz); }
	 * if(ball.y<-25||ball.y>25){ ball.setVelocity(ball.velx,-1*ball.vely,
	 * ball.velz); } for(PingPongWebSocket ws:sockets)
	...
	/*
	 * public void stop(){ timer.cancel(); started = false; }
	 */

	/*
	 * public boolean isStarted(){ return started; }
	 * 
	 * public void addWebSocket(PingPongWebSocket ws){ sockets.add(ws);
	 * System.out.println(sockets.size()); } public void
	 * removeWebSocket(PingPongWebSocket ws){ sockets.remove(ws); }
	 */

	public GameUpdate getMessage() {
		/*
		 * String result=""; result+=ball.x; result+=" "; result+=ball.y;
		 * result+=" "; result+=ball.z; result+=" "; result+=paddle1.x;
		 * result+=" "; result+=paddle1.y; result+=" "; result+=paddle2.x;
		 * result+=" "; result+=paddle2.y; result+=" ";
